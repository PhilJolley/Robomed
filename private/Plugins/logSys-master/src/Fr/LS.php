...
class LS
{
    ...
    public static $default_config = array(
         ...
         */
        'basic' => array(
            'company'        => 'Robomed Network',
            'email'          => 'phillip.jordan.31@gmail.com',
...
HTML;
    ...
    public function getUserByToken($reset_pass_token){
    	 $sql = $this->dbh->prepare('SELECT uid FROM ' . $this->config['db']['token_table'] . ' WHERE token = ?');
         $sql->execute(array($reset_pass_token));
         $userID = $sql->fetchColumn();
         return $userID;
    ...
    public function sendResetPasswordToken($uid, $messageCallback = false)
    {
        ...
        if (is_callable($messageCallback)) {
            ...
            $body = '<html><body><p>You requested for resetting your password on ' . $this->config['basic']['company'] . '. For this, please click the following link :</p>
            <blockquote>
    ...
    public function sendMail($email, $subject, $body)
    {
		...
		$headers .= 'From: Robomed Network < info@robomedcampaign.com '.">\r\n".
